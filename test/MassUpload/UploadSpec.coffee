...
describe 'MassUpload/Upload', ->
  ...
  date2 = new Date('Mon, 12 Aug 2013 11:02:54 -0400').valueOf()
  ...
  describe 'starting with a File', ->
    ...
    it 'should have lastModified, memoized', ->
      expect(subject.lastModified()).to.eq(date1)
      expect(subject.lastModified()).to.eq(date1)  # memoized
    ...
    describe 'updateWithProgress', ->
      ...
      it 'should create a fileInfo', ->
        ...
        expect(fileInfo.lastModified).to.eq(file.lastModified)
  ...
  describe 'starting with an incomplete FileInfo', ->
    fileInfo = { loaded: 2000, total: 10000, name: 'file.txt', lastModified: date1 }
    subject = undefined
  ...
  describe 'with incompatible File and FileInfo', ->
    file = { size: 12000, name: 'file.txt', lastModified: date2 }
    fileInfo = { loaded: 10000, total: 10000, name: 'file.txt', lastModified: date1 }
    subject = undefined
    beforeEach -> subject = new Upload({ file: file, fileInfo: fileInfo })
  ...
  describe 'hasConflict', ->
    ...
    it 'should be false when File and FileInfo match', ->
      init(
        { size: 10000, name: 'file.txt', lastModified: date1 },
        { loaded: 10000, total: 10000, name: 'file.txt', lastModified: date1 }
      )
      expect(subject.hasConflict()).to.eq(false)

    it 'should be true when size is different', ->
      init(
        { size: 10003, name: 'file.txt', lastModified: date1 },
        { loaded: 10000, total: 10000, name: 'file.txt', lastModified: date1 }
      )
      expect(subject.hasConflict()).to.eq(true)

    it 'should be true when lastModified is different', ->
      init(
        { size: 10000, name: 'file.txt', lastModified: date2 },
        { loaded: 10000, total: 10000, name: 'file.txt', lastModified: date1 }
      )
      expect(subject.hasConflict()).to.eq(true)
